    ...
    return (
        <div className="min-h-screen bg-neutral-50 p-8">
            <div className="mb-8 flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-neutral-900">Transaksi Peminjaman</h1>
                    <p className="text-neutral-600">Kelola peminjaman dan pengembalian buku</p>
                </div>
                <Link
                ...
                <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-neutral-100">
                        {loading ? (
                            ...
                            transactions.map((t) => (
                                <tr key={t.id} className="hover:bg-neutral-50 cursor-pointer" onClick={() => navigate(`/dashboard/transactions/${t.id}`)}>
                                    <td className="px-6 py-4 font-mono text-neutral-500">#{t.id}</td>
                                    <td className="px-6 py-4 font-medium text-neutral-900">{getStudentName(t.studentId)}</td>
                                    <td className="px-6 py-4 text-neutral-600">
                                        {new Date(t.borrowedAt).toLocaleDateString('id-ID')}
                                    </td>
                                    <td className="px-6 py-4 text-neutral-600">
